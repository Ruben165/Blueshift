...
@section('title', 'Edit Batch Mitra')
...
@section('content_header')
    ...
    <div class="content_header">
        <h1>Edit Batch Mitra {{ $batch_mitra->name }}</h1>
    </div>
@endsection
...
@section('content')
    <div class="row">
        <div class="col-md-3">
            @include('master.group.__formAddPartner')
            @include('master.group.__formEdit')
        </div>
        <div class="col-md-9">
            @include('master.group.__tablePartner')
        </div>
    </div>
@endsection
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).ready(function() {
                $('#clinic_id').select2({
                    ajax: {
                        url: "{{ route('batch-mitra.get-all-partners') }}",
            ...
            $(document).on('click', '.Hapus', async function(){
                swal.fire({
                    ...
                    text: "Mitra ini akan dihapus dari batch {{ $batch_mitra->name }}!",
                    icon: 'warning',
        ...
        function initPartnerDatatable(){
            let table = $('#partners-list').DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route($route, ['batch_mitra' => $batch_mitra->id]) }}",
                "columnDefs": [
                    {
                        "data": "name",
                        "name": "name",
